...
# Tsunami alerts come from the two US tsunami warning centers (NTWC and
# PTWC) and cover whole ocean basins, not a single NWS forecast office,
# so all tsunami entities are grouped under one dedicated global device
# following the same pattern as hurricanes above.
TSUNAMI_DEVICE_ID = "noaa_tsunami"
TSUNAMI_DEVICE_NAME = "NOAA Tsunami"
...
# Tsunami endpoints.
#
# The NWS alerts query filters on VTEC-derived codes rather than event
# names: the code is the two-character phenomenon (TS) plus the one
# character significance (W/A/Y), which is stable across NWS wording
# changes in a way that matching the string "Tsunami Warning" is not.
NWS_TSUNAMI_ALERTS_URL = "https://api.weather.gov/alerts/active?code=TSW,TSA,TSY"

# The two US tsunami warning centers publish Atom feeds of their event
# products. NTWC (Palmer, AK) covers the US and Canadian mainland coasts
# plus Puerto Rico; PTWC (Ewa Beach, HI) covers Hawaii, the Pacific
...
# Each center also publishes its current event as a CAP 1.2 document at a
# stable URL alongside the Atom feed. This is where the per-location estimated
# wave arrival times live, which the Atom feed does not carry.
TSUNAMI_CAP_URLS = {
...
#
# The NWS alert query runs every cycle because it is what the threat level and
# the alert binary sensor read. The center feeds are only consulted when an
# alert is actually active, or once every this-many cycles to keep the product
# list and the monthly test message fresh. At TSUNAMI_SCAN_INTERVAL that is
# roughly every half hour, which keeps a permanently-idle install from making
# 3,600 requests a day to tsunami.gov for a feed that has not changed.
TSUNAMI_CENTER_POLL_EVERY = 15
...
# Tsunami alert tuning.
#
# Polled far more often than DEFAULT_SCAN_INTERVAL. A near-field tsunami — one
# generated by an earthquake just offshore — can reach the coast in under
# fifteen minutes, so a ten-minute poll could burn most of the warning time
# before the sensor ever changed state. The payloads are a few kilobytes.
TSUNAMI_SCAN_INTERVAL = 2  # minutes

# Minutes without a successful fetch before the data-stale binary sensor turns
# on. A threat-level sensor reading "None" from a feed that died an hour ago is
# worse than no sensor at all, so staleness is surfaced as its own entity.
TSUNAMI_STALE_AFTER = 15  # minutes

# The four US tsunami alert levels, highest first. Passed into the parsers
# rather than imported by them, matching how AURORA_KP_THRESHOLDS and the
# meteor catalog are injected.
TSUNAMI_THREAT_LEVELS = ("Warning", "Advisory", "Watch", "Information")

# Coastal offices only, mapped to the warning center responsible for them.
# Presence in this table is what gates the location-specific tsunami entities,
# the same way OFFICE_RADAR_SITES gates the radar entities. The Great Lakes
# offices (APX, CLE, DLH, DTX, GRB, GRR, IWX, LOT, MKX, MQT) are deliberately
# absent: they have no tsunami exposure and get only the global entities.
OFFICE_TSUNAMI_CENTERS = {
    # US West Coast
    "EKA": "NTWC",  # Eureka, CA
    "LOX": "NTWC",  # Los Angeles/Oxnard, CA
    "MFR": "NTWC",  # Medford, OR
    "MTR": "NTWC",  # San Francisco/Monterey, CA
    "PQR": "NTWC",  # Portland, OR
    "SGX": "NTWC",  # San Diego, CA
    # Pacific islands
    "GUM": "PTWC",  # Guam
    "HFO": "PTWC",  # Honolulu, HI
    # Caribbean
    "SJU": "PTWC",  # San Juan, PR
    # US East Coast
    "AKQ": "NTWC",  # Wakefield, VA
    "BOX": "NTWC",  # Boston, MA
    "CAR": "NTWC",  # Caribou, ME
    "CHS": "NTWC",  # Charleston, SC
    "GYX": "NTWC",  # Portland, ME
    "ILM": "NTWC",  # Wilmington, NC
    "MHX": "NTWC",  # Morehead City, NC
    "OKX": "NTWC",  # New York, NY
    "PHI": "NTWC",  # Philadelphia, PA
    # Gulf of Mexico and Florida
    "BRO": "NTWC",  # Brownsville, TX
    "CRP": "NTWC",  # Corpus Christi, TX
    "JAX": "NTWC",  # Jacksonville, FL
    "MFL": "NTWC",  # Miami, FL
    "MLB": "NTWC",  # Melbourne, FL
    "MOB": "NTWC",  # Mobile, AL
    "TAE": "NTWC",  # Tallahassee, FL
    "TBW": "NTWC",  # Tampa Bay, FL
}
